...
# 2022.12.14 Update: quick and dirty update to add auth. It will prompt for password. See below:

# usage: ./rpcfiend.sh 192.168.1.1

if [ $# -eq 0 ]; then
        echo "No parameters supplied."
        echo "Usage: ./rpcfiend.sh 192.168.1.1 username"
        exit 1
fi
...
TEST=`rpcclient -U "${username}%${password}" $HOST -c "getdompwinfo;exit"`
response_code=$?

echo $TEST
echo "Result was: $response_code"

if [ $response_code == 0 ]; then
        echo "Successful test of credentials... proceeding."
else
        echo "Creds didn't work, calling it off."
        exit
fi
...
function get_domain_admins(){
        ...
        DA_RID_ARRAY=`rpcclient -U "${username}%${password}" $HOST -c "querygroupmem 0x200" | sed 's/rid:\[//g' | tr -d ']' | sed 's/attr:\[0x7//g'`
        DA_ARRAY=`rpcclient -U "${username}%${password}" $HOST -c "samlookuprids domain $DA_RID_ARRAY"`
...
function get_domain_controllers(){
        DC_RID_ARRAY=`rpcclient -U "${username}%${password}" $HOST -c "querygroupmem 0x204" | sed 's/rid:\[//g' | tr -d ']' | sed 's/attr:\[0x7//g'`
        DC_ARRAY=`rpcclient -U "${username}%${password}" $HOST -c "samlookuprids domain $DC_RID_ARRAY"`
...
function get_domain_machines(){
        DM_RIDS=`rpcclient -U "${username}%${password}" $HOST -c "querygroupmem 0x203" | sed 's/rid:\[//g' | tr -d ']' | sed 's/attr:\[0x7//g'`
        ...
        while [ $arrayend -lt $maxarraycount ]; do
                ...
                rpcclient -U "${username}%${password}" $HOST -c "samlookuprids domain $tempstring" | cut -d ' ' -f3 | tee -a rpcfiend_domain_machines.txt
